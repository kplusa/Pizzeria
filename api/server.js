...
app.post("/create-order", function (req, res) {
  ...
  if (req.body.delivery == "no")
    sql =
      "INSERT INTO pizza_order ( delivery, pizza_type, pizza_size, pizza_dough, pizza_sauce, amount_of_pizzas) VALUES ('" +
      req.body.delivery +
      "', '" +
      req.body.pizzaType +
      "', '" +
    ...
    sql =
      "INSERT INTO pizza_order ( delivery, fullname, telephone_number, street,city, pizza_type, pizza_size, pizza_dough, pizza_sauce, amount_of_pizzas) VALUES ('" +
      req.body.delivery +
      "','" +
      req.body.fullname +
      "','" +
...
app.get("/orders/:orderId", function (req, res) {
  var sql = "";
  sql = "SELECT * FROM `pizza_order` where id='" + req.params.orderId + "'";
  con.query(sql, [], function (err, result) {
    if (err) throw err;
    res.json(result);
  });
